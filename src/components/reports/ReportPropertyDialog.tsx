...
const ReportPropertyDialog = ({ propertyId }: ReportPropertyDialogProps) => {
  ...
  const { reportProperty, loading } = useReports();
  const { propertyReportTypes } = useReportTypes();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await reportProperty(propertyId, {
      ...
      <DialogTrigger asChild>
        ...
        >
          ...
          <span className="font-medium select-none hidden lg:inline">
            {currentLanguage === 'ar' ? 'إبلاغ' : currentLanguage === 'tr' ? 'Bildir' : 'Report'}
          </span>
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {currentLanguage === 'ar' ? 'إبلاغ عن العقار' : 'Report Property'}
          </DialogTitle>
          <DialogDescription>
            {currentLanguage === 'ar' 
